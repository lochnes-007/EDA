...
def _resolve_data_path(path):
    """Resolve a data file path relative to the workspace or the module directory."""
    if path is None:
        return None
...
def read_stata(dct_file, dat_file, **options):
    ...
    """
    dct_path = _resolve_data_path(dct_file)
    dat_path = _resolve_data_path(dat_file)
    stata_dict = parse_stata_dict(dct_path)
...
def read_fem_resp2010():
    ...
    df = read_fem_resp(
        "./data/2006_2010_FemRespSetup.dct", "./data/2006_2010_FemResp.dat.gz", usecols=usecols
    )
    df["evrmarry"] = df.evrmarry == 1
...
def read_fem_resp2013():
    ...
    df = read_fem_resp(
        "./data/2011_2013_FemRespSetup.dct", "./data/2011_2013_FemRespData.dat.gz", usecols=usecols
    )
    df["evrmarry"] = df.evrmarry == 1
...
def read_variables():
    ...
    """
    vars1 = parse_stata_dict("./data/2002FemPreg.dct").names
    vars2 = parse_stata_dict("./data/2002FemResp.dct").names
